...
let winnerIdentified = false  // assigned as false to let the loop continue if there's no winner
let currentTurnPlayer = 'X'   //'X' is act as the first player.

while (!winnerIdentified){
    let player = playTurn(currentTurnPlayer);
    //feel free to add logic here if needed, e.g. announcing winner or tie
    
    if (player == 'X'){
        console.log(player + " win! Congratulations!!")
        var input = prompt("Do you want to play again? (Y/N)")
        playAgain(input,winnerIdentified) 
        }
